...
////__FHY_ADD_BEGIN__
#define check_sat_assuming 0
bool union_flag = false;
////__FHY_ADD_END__

#if check_sat_assuming
//__LWT_ADD_BEGIN__
static unsigned int CLK_COMPARE_COUNTER = 0;
//__LWT_ADD_END__
#endif
...
void smt2_convt::write_header() {
    ...
    switch (solver) {
        ...
        case solvert::BOOLECTOR:
            out << "; Generated for Boolector\n";
            break;
        case solvert::CVC3:
            out << "; Generated for CVC 3\n";
            break;
        case solvert::CVC4:
            out << "; Generated for CVC 4\n";
            break;
        case solvert::MATHSAT:
            out << "; Generated for MathSAT\n";
            break;
        case solvert::OPENSMT:
            out << "; Generated for OPENSMT\n";
            break;
        case solvert::YICES:
            out << "; Generated for Yices\n";
            break;
        case solvert::Z3:
            out << "; Generated for Z3\n";
            break;
    }
...
void smt2_convt::write_footer(std::ostream &out) {
    ...
    for (unsigned int i = 0; i < CLK_COMPARE_COUNTER; ++i) {
        out << "tmp" << i << " ";
    }
    out << "))\n";
#else
...
/**
 * 根据expr的id来判断类型，然后分别调用子方法来convert expr。
 * @param expr
 */
    ...
    if (expr.id() == ID_symbol) {
        ...
        if (use_FPA_theory) {
            ...
            convert_floatbv(expr);
               ...
               expr.id() == ID_lt ||
               expr.id() == ID_ge ||
               expr.id() == ID_gt) //the if block for > >=, < ,<= exprs.
    {
      ...
      out << ")) bin1)"; // bvlshr, extract, =
        ...
        if(!union_flag)
        {
            ...
            boolbv_widtht bool_width(ns);
            unsigned width = bool_width(expr.type());
            out << "(_ bv0 " << width << ")";
        }
        //// __FHY_ADD_END__
    } else if (expr.id() == ID_literal) {
...
void smt2_convt::convert_typecast(const typecast_exprt &expr) {
    ...
    if (dest_type.id() == ID_bool) {
        ...
        if (src_type.id() == ID_signedbv || // from signedbv
            ...
            {
                //// __FHY_ADD_BEGIN__
                if(from_expr(ns, "", src)=="-1" && src_type.id() == ID_signedbv)
                    out << "((_ zero_extend ";
...
void smt2_convt::convert_with(const with_exprt &expr) {
    ...
    if (expr_type.id() == ID_array) {
        ...
        if (use_array_theory(expr)) {
            out << "(store ";
//            std::cout << "get array type: " << array_type.size().type().id_string() << "\n";
            convert_expr(expr.old());
            out << " ";
            convert_expr(typecast_exprt(expr.where(), array_type.size().type()));
            out << " ";
//            std::cout << "get array element type: " <<from_expr(expr.new_value()) << "\n";

            // __FHY_ADD_BEGIN__
            if(array_type.size().type().id() == ID_signedbv &&
               expr.new_value().type().id()==ID_bool && !use_array_of_bool)
            {
              if(expr.new_value().is_true())
              {
                out << "(_ bv1 1)";
              }
              else if (expr.new_value().is_false())
              {
                out << "(_ bv0 1)";
              }
            }
...
void smt2_convt::set_to(const exprt &expr, bool value) {
    if (expr.id() == ID_and && value) {
        forall_operands(it, expr)set_to(*it, true);
        return;
    }

    if (expr.id() == ID_or && !value) {
        forall_operands(it, expr)set_to(*it, false);
        return;
    }
    ...
    if (expr.id() == ID_equal && value == true) {
        ...
        if (equal_expr.lhs().id() == ID_symbol) {
            ...
            if (identifier_map.find(identifier) == identifier_map.end()) {
                ...
                //// __FHY_ADD_BEGIN__
                const irep_idt& lhs_id = ns.follow(equal_expr.lhs().type()).id();
                if(lhs_id == ID_struct || lhs_id == ID_union ||
                   lhs_id == ID_struct_tag || lhs_id == ID_union_tag)
                    union_flag = true;
...
#if check_sat_assuming
        //__LWT_ADD_BEGIN__
        if (expr.id() == ID_ge && expr.op0().id()==ID_symbol && expr.op1().id()==ID_symbol) {
            PRECONDITION(expr.id()==ID_ge && expr.operands().size()==2);
            std::string key_word = "clk";
            bool is_clock_compare = false;
            std::string name_op_0 = convert_identifier(to_symbol_expr(expr.op0()).get_identifier());
            std::string name_op_1 = convert_identifier(to_symbol_expr(expr.op1()).get_identifier());
            bool clk_in_op0 = strstr(name_op_0.c_str(), key_word.c_str());
            bool clk_in_op1 = strstr(name_op_1.c_str(), key_word.c_str());
            is_clock_compare = clk_in_op0 && clk_in_op1;

            if (is_clock_compare) {
                out << "(declare-const tmp"<<CLK_COMPARE_COUNTER<<" Bool)" <<"\n";
                out << "(assert ( = tmp"<<CLK_COMPARE_COUNTER++<<" ( < |"<< name_op_0 <<"| |"<<name_op_1 <<"| ))";
            } else {
                out << "(assert (not ";
                convert_expr(expr);
                out << ")";
...
void smt2_convt::find_symbols(const exprt &expr) {
    ...
    forall_operands(it, expr)find_symbols(*it);
...
void smt2_convt::collect_bindings(
    ...
    Forall_operands(it, expr)collect_bindings(*it, map, let_order);
...
exprt smt2_convt::substitute_let(
    ...
    Forall_operands(it, expr)it->visit(lv);
