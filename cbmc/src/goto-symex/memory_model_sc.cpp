      ...
      a_it++)
  {
        ...
        ++w_it1)
    {
          ...
          ++w_it2)
      {
        ...
        // __FHY_ADD_BEGIN__
        unsigned int w1_thread_no = (*w_it1)->source.thread_nr;
        unsigned int w1_intra_thread_index = numbering[*w_it1];
        unsigned int w2_thread_no = (*w_it2)->source.thread_nr;
        unsigned int w2_intra_thread_index = numbering[*w_it2];

        std::string key_word = "__CPROVER";
        bool is_lib_var = strstr(a_it->first.c_str(), key_word.c_str());
        std::string choice_ws_var_name =
          "ws-ext_" + std::to_string(w1_thread_no) + "_" + std::to_string(w1_intra_thread_index)
          + "_" + std::to_string(w2_thread_no) + "_" + std::to_string(w2_intra_thread_index)
          + "_" + (is_lib_var ? "T" : "F") + "_";

        symbol_exprt s=nondet_bool_symbol(choice_ws_var_name);
